...
def max_swaps(results: Iterator, range_amounts: tuple) -> list[Swap]:
    """
    ...
    :param range_amounts: Range amounts for swaps
    :return: List [swap_data]
    """
    results_list = list(results)
    swaps_amounts = []
    for amount in range(*range_amounts):

        # Create dict{swap_max_amount: swap_data} if swap not None
        swaps = {swap.to_token.amount: swap for swap in results_list
                 if swap and swap.from_token.amount == amount}

        max_amount = max(swaps)
        swap = swaps[max_amount]
        swaps_amounts.append(swap)

    return swaps_amounts


def compare_swaps(data: dict, base_token: str, arb_token: str) -> List[Swap]:
    """
    ...
    """
    # Query all networks for Base->Arb swap out
    ...
    # Get the maximum Swap and use only 1 amount

    all_max_swaps = []
    for max_swap_ab in max_swaps(swaps_ab, ranges):
        max_amount_ab = max_swap_ab.to_token.amount

        # Query networks for Arb->Base swap out
        args_ba = parse_args_1inch(data, arb_token, base_token, (max_amount_ab, ))
        ...
        max_swap_ba = max_swaps(swaps_ba, ranges)[0]
