...
def check_arb(data: dict, base_token: str, arb_token: str) -> None:
    """
    Checks 1inch supported blockchains for arbitrage between 2 tokens and alerts via Telegram.
    ...
    # Get the maximum swap out
    max_swap_ba, max_amount_ba = max_swap(results)

    base_round = int(max_swap_ab['from_token']['decimals'] / 3)
    arb_round = int(max_swap_ab['to_token']['decimals'] / 3)

    base_swap_in = round(max_swap_ab['from_token']['amount'], base_round)
    arb_swap_out = round(max_swap_ab['to_token']['amount'], arb_round)
    network_1 = max_swap_ab['network']
    base_swap_out = round(max_swap_ba['to_token']['amount'], base_round)
    arb_swap_in = round(max_swap_ba['from_token']['amount'], arb_round)
    network_2 = max_swap_ba['network']

    arbitrage = max_amount_ba - base_swap_in
    ...
    if arbitrage >= min_arb:
        ...
        telegram_msg = f"{timestamp}\n" \
                       ...
                       f"<a href='https://app.1inch.io/#/1/swap/{base_token}/{arb_token}'>" \
